 ...
 */
class TitleResolverCompilerPass implements CompilerPassInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (!$container->has(TitleResolver::class)) {
            return;
        }

        $definition = $container->findDefinition(TitleResolver::class);

        $taggedServices = $container->findTaggedServiceIds('ddr_crud_admin.title_provider');
        $prioritizedIds = [];
